...
def get_embedding(cfg):
    ...
    if  cfg['embedding'] == 'word2vec':
        class_dict = np.load(dataset_path+'/word_vectors/word2vec_addback.npy', allow_pickle=True).tolist()
        class_emb = class_dict[0]
        for i in range(1, cfg['all_classes'] + 1):
            class_emb = np.row_stack((class_emb,class_dict[i]))
...
def get_split(cfg):
    if 'zeroseg' not in cfg['dataset']:
        dataset_path = os.path.join(cfg['datadir'], cfg['dataset'])    #./dataset/zeroseg
        ...
        #array.dict -> keys_list
        seen_classes = np.load(dataset_path + '/split/seen_cls.npy', allow_pickle=True).tolist().keys()
        all_classes = np.load(dataset_path + '/split/all_cls.npy', allow_pickle=True).tolist().keys()
        novel_classes = [i for i in all_classes if i not in seen_classes]

        #keys_list -> numpy
        seen_classes = np.array(list(seen_classes))
        all_classes = np.array(list(all_classes))
        novel_classes = np.array(novel_classes)
        seen_novel_classes = np.concatenate((seen_classes, novel_classes), axis=0)

        #all label name
        all_labels = np.array(list(np.load(dataset_path + '/split/all_cls.npy', allow_pickle=True).tolist().values()))
