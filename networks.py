...
class MSCC(nn.Module):
    ...
    def forward(self, x, mask=None):
        if mask is not None:
            ...
            logits_h0 = self.get_resized_logits(x, h0, mask, 1)
            return KLD, logits_h0, h0, h1

        else:
            h0, h1 = self.scale(x)
            logits_h0 = self.get_resized_logits(x, h0, mask=None, scale_return_index=1)
            return logits_h0, h0, h1



    def get_resized_logits(self, x, logits, mask=None, scale_return_index=1):
        # Original
        interp = lambda l: F.interpolate(l, size=logits.shape[2:], mode="bilinear", align_corners=False)

        # Scaled
        logits_pyramid = []
        for p in self.pyramids:   #[0.5, 0.75]
            ...
            if mask is not None:
                ...
                logits_pyramid.append(self.scale(h)[scale_return_index])
...
class DeepLabV2_local(nn.Sequential):
    ...
    def forward(self, x, mask=None):
        ...
        localmu = F.interpolate(self.contextuallocalmu(h1), size=h.size()[2:], mode="bilinear", align_corners=False)
        localsigma = F.interpolate(self.contextuallocalsigma(h1), size=h.size()[2:], mode="bilinear", align_corners=False)
        h1 = F.interpolate(self.reparameterize(localmu, localsigma), size=h.size()[2:], mode="bilinear", align_corners=False) # contextual latent code
        att = torch.sigmoid(h1)
        h_att = torch.mul(h, att)
        h0 = torch.sigmoid(h + h_att) # augmented feature X_n
        ...
        if mask is not None:
            ...
            return h0, h1 # KLD loss, feature, contextual latent code
