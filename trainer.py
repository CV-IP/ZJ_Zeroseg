...
class Trainer(nn.Module):
    ...
    def train(self, data, gt, mode, multigpus):
        ...
        if mode == 'step1':
            flag, loss_G_GAN, loss_G_Content, \
            loss_G_cls, loss_G, loss_B_KLD, \
            loss_B_cls, loss_B, loss_D_real, \
            loss_D_fake, loss_D_gp, loss_D_GAN, \
            loss_D_cls_real, loss_D_cls_fake, loss_D_cls, \
            loss_D, loss_cls_real, loss_cls_fake, acc_cls_real, acc_cls_fake = self.model(data, gt, mode)
        else:
    ...
    def val(self, data, gt, multigpus):
        with torch.no_grad():
            this_model = self.model.module if multigpus else self.model

            flag, pred_cls_real, sorted_indices, resized_gt, resized_ignore_mask = this_model.val(data, gt)
            if (flag == -1).any() == True:
                raise MeaninglessError()
    ...
    def test(self, data, multigpus):
        with torch.no_grad():
            this_model = self.model.module if multigpus else self.model
            pred_cls_real = this_model.test(data)
            return pred_cls_real
